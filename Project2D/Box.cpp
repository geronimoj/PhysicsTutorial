...
Box::Box(glm::vec2 position, bool isKinematic, glm::vec2 velocity, float mass, glm::vec4 colour, glm::vec2 extents, float orientation, float angularVelocity, unsigned int layer, float linearDrag, float angularDrag, float elasticity, float staticFriction, float kinematicFriction)
	: Rigidbody(ShapeType::BOX, position, isKinematic, velocity, layer,orientation, mass, elasticity, angularVelocity, linearDrag, angularDrag, staticFriction, kinematicFriction), m_colour(colour), m_extents(extents), m_localX(glm::vec2(0,0)), m_localY(glm::vec2(0,0))
{
